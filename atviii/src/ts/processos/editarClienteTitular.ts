...
export default class EditarClienteTitular extends Processo {
    ...
    processar(): void {
        ...
        if(rua) cliente.Endereco.Rua = rua
        if(bairro) cliente.Endereco.Bairro = bairro
        if(cidade) cliente.Endereco.Cidade = cidade
        if(estado) cliente.Endereco.Estado = estado
        if(pais) cliente.Endereco.Pais = pais
        if(codigoPostal) cliente.Endereco.CodigoPostal = codigoPostal
